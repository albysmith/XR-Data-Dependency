            ...
            for entry in fs::read_dir(&folder.catpath).expect("read_dir") {
                        ...
                        .contains("sig")
                {
                    ...
                    for line in catfile_string.lines() {
                        ...
                        if (path.contains("assets/units/") && (path.contains("data") || path.contains("DATA"))) ||
                           (path.contains("EngineSystems") && (path.contains("data") || path.contains("DATA"))) ||
                           (path.contains("StorageModules") && (path.contains("data") || path.contains("DATA"))) 
...
fn comp_mod_files(folder: &Dir, out_path: &Path) {
    ...
    for file in folder.files() {
        ...
        content_file.write_all(&file.contents_utf8().expect("reading mod file").as_bytes()).expect("writing mod file");
    }
